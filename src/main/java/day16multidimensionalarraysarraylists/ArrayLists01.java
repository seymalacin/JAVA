...
public class ArrayLists01 {
    public static void main(String[] args) {
        ...
        //3.Yol:
        List<Integer> nums = new ArrayList<>();//****** en tercih edien yol
        ...
        nums.add(1, 50);//indexe eeman vermemize yarar
        ...
        nums.addAll(2, prices);// list icine list ekeemize yarar
        System.out.println(nums);// [21, 50, 23, 185, 18, 20, 23, 185]


        //ArrayList'lerin eleman sayısı nasi bulunur?
        int elemanSayisi = nums.size(); //size() arraylar de "lenght" listlerde "size"
        System.out.println(elemanSayisi);//8

        //ArrayList'lerde herhangi bir eleman nasil secilir?
        // get() methodu istenen index'teki elemani verir
        int el1 = nums.get(3);// get()ısdenen bır indexın eemanini verir
        System.out.println(el1);//185

        //ArrayList'in bos olup olmadigini nasil anlariz ?
        boolean bos1 = nums.isEmpty(); //  boolean =>bos odugu zaman "true dolu odu zaman "fase"
        System.out.println(bos1);//fase

        boolean bos2 = ages.isEmpty();
        System.out.println(bos2);//true

        //ArrayList'de bir eleman nasil degistirilir?
        nums.set(3, 200);// var olan eamani yeni elemana degistirmeye yarar
        System.out.println(nums);//[21, 50, 23, 200, 18, 20, 23, 185]

        //Example 1: nums ArrayList'indeki tum tek sayilari 11 artırdıktan sonra ekrana yazdiriniz

        for (int w : nums) {
            if (w % 2 != 0) {
                nums.set(nums.indexOf(w), w + 11);// sasdece update olanları ekrana yazdırmaya yarar
        ...
        //remove() methodunu icine tam sayı koyarsaniz java onu index oarak kabul eder
        ...
        //Example 2:ArrayList'den 200 elemani siliniz
        Integer sayı = 200;
        nums.remove(sayı);
